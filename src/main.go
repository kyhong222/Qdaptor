...
func main() {
	...
	api.Heartbeat()
	time.Sleep(1 * time.Second)
	api.Login(g_agentID, g_DN, g_tenant)
	time.Sleep(1 * time.Second)
	api.Heartbeat()
	time.Sleep(1 * time.Second)
	api.Heartbeat()
	time.Sleep(1 * time.Second)
	api.SetReady(g_tenant, g_agentID)
	time.Sleep(1 * time.Second)
	api.SetAfterCallReady(g_tenant, g_agentID)
	time.Sleep(1 * time.Second)
	api.Heartbeat()
	time.Sleep(1 * time.Second)
	api.HeartbeatMaker(HBPeriod)
